...
def  get_parking_location_status():
    ...
    response = requests.post(url=url_getParkingLocationStatusList,headers=headers,data=json.dumps(body))
...
#    conn.close()
    ...
    print("count current monitoring list: ", len(current_platenumber_list))
    # sys.exit(0)

    # AMANO DB에서 현재시간부터 12시간 전가지의 출차처리되지 않은 입차된 차량 현황 조회 - 주차관제 모니터링 후보군
    datetime_now = datetime.now()
    datetime_start = datetime_now - timedelta(hours=24)
    ...
    print(f'amano db result length : {len(mssql_db_result)}')
    count_monitored = 0
    for k,row in enumerate(mssql_db_result):
        ...
        if platenumber_orig in current_platenumber_list:
            ...
            search_target_list.append(platenumber_orig)
#         else:
...
#                 pass
    print("count already in monitoring list", count_monitored)
    # sys.exit(0)
    ...
    """
    # 차량 번호 후보군 조회
    for plate_full in search_target_list:
        platenumber = plate_full[-4:]
        # plate_number_str = f"{plate_number:04d}"
        #print(plate_number_str)

        api_start = time.time()
        # 네트워크 트래픽 확인
        body["carNo4Digit"] = platenumber
        ...
        if car_loc["status"] == "200" and car_loc["data"]["success"]:
            ...
            for car in car_loc["data"]["carList"]:
                ...
                result = {
                    ...
                    "targetNumber": plate_full
                }
                overall_result.append(result)
                if carNo in current_platenumber_list:
                    # print(f'현재{plate_full} 조회. {carNo}는 이미 모니터링 대상')
                    count_same_number += 1
                    continue
    ...
    print("count_no_loc", count_no_loc)
    print("count_tower", count_tower)
    print("count_same_number", count_same_number)

    csv_file_path = "sync_monitoring_3_1.csv"

    with open(csv_file_path, mode="w", newline="", encoding="utf-8") as file:
        writer = csv.DictWriter(file, fieldnames=["plateNumber", "parkingPosition", "enterTime", "targetNumber"])
        
        # 헤더 작성
        writer.writeheader()
        
        # 데이터 작성
        writer.writerows(overall_result)
"""
#                 # 주차관제 DB에서 차량 정보 조회
...
def print_loc(response):
    # 데이터 추출
    loc_list = response["data"]["locList"]

    # 전체 카운트
    total_count = {"true": 0, "false": 0}

    # levelNo별 카운트
    level_count = defaultdict(lambda: {"true": 0, "false": 0})

    # 데이터 처리
    for loc in loc_list:
        level_no = loc["levelNo"]
        status = "true" if loc["currentStatus"] else "false"

        # 전체 카운트 증가
        total_count[status] += 1

        # levelNo별 카운트 증가
        level_count[level_no][status] += 1

    # 결과 출력
    print("Total Count:")
    print(f"  True: {total_count['true']}")
    print(f"  False: {total_count['false']}")

    print("\nLevelNo-wise Count:")
    for level_no, counts in level_count.items():
...
if __name__ == "__main__":
    ...
    status = get_parking_current_status()
    location = get_parking_location_status()
    print(status)
    print_loc(location)
    # print(location)
    # 전체 결과를 저장할 리스트
