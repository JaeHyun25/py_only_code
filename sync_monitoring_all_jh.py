...
def process():
    ...
    overall_result = [] # 결과를 저장할 리스트 초기화   # 내가 추가
    count = 0 # 추출한 차량 대수 카운터 # 내가 추가
    # 유종 기본값 설정
...
#                        vehicle_type = "Unknown"
                    result = {
                        ...
                        "enterTime": parkingTime,
                        #"powertrainTypeCode": powertrainTypeCode,
                        "powertrainTypeCode": base64.b64encode(powertrainTypeCode).decode('utf-8') # bytes를 Base64로 인코딩 후 문자열로 디코딩
                        
                    }
                print(result)
                overall_result.append(result)
                count += 1      # 5줄 내가 작성
                if count >= 5: # 10대 추출 완료 시 루프 종료
                    break
        if count >= 5: # 외부 루프도 종료
            break
    ...
    return overall_result # 추출된 차량 정보 리스트 반환    # 내가 추가가
    #sys.exit(0)    # 내가 수정정
...
if __name__ == "__main__":
    ...
    # 전체 결과를 저장할 리스트
    #overall_result = []    # 내가 수정 - 이 줄은 process() 함수 내에서 초기화하도록 변경
    extracted_car_info = process()
    print("\n---추출된 차량 정보 5대 ---")
    print(json.dumps(extracted_car_info, indent=4, ensure_ascii=False)) # console에도 json형태로 출력
    with open("extracted_cars.json", "w", encoding="utf-8") as f:
        json.dump(extracted_car_info, f, indent=4, ensure_ascii=False)

    # for car in extracted_car_info:
    #     print(car)
    #overall_result = process() # 내가 수정
    sys.exit(0) # 내가 수정